...
;------------------------------
;Actions to be taken when a threshold level is reached
;#mac VMON_VBAT_HVACTION, 0
;	...code to be executed when the upper threshold is reached
;#emac
;#mac VMON_VBAT_LVACTION, 0
;	...code to be executed when the upper threshold is reached
;#emac
...
;--------------------------
;Actions to be taken when a threshold level is reached
;#mac VMON_VUSB_HVACTION, 0
;	...code to be executed when the upper threshold is reached
;#emac
;#mac VMON_VUSB_LVACTION, 0
;	...code to be executed when the upper threshold is reached
;#emac
...
VMON_ISR			EQU	*
				...
				;Check VUSB
				;==========
				BRCLR	ATDSTAT2L,  #VMON_VUSB_CONVERSION, VMON_ISR_1 	;skip if state hasn't changed
				BRSET	ATDCMPHTL, #VMON_VUSB_CONVERSION, VMON_ISR_2 	;HV condition detected
				...
				VMON_VUSB_LVACTION
#endif				
VMON_ISR_1			JOB	VMON_ISR_3					;VUSB check done
				;HV condition detected
VMON_ISR_2			BCLR	ATDCMPHTL, #VMON_VUSB_CONVERSION   		;VUSB must be lower (or same) than threshold
...
VMON_ISR_3			EQU	*
				...
				;Check VBAT
				;==========
				BRCLR	ATDSTAT2L,  #VMON_VBAT_CONVERSION, VMON_ISR_4 	;skip if state hasn't changed
				BRSET	ATDCMPHTL, #VMON_VBAT_CONVERSION, VMON_ISR_5 	;HV condition detected
				...
				VMON_VBAT_LVACTION
#endif				
VMON_ISR_4			JOB	VMON_ISR_6					;VBAT check done
				;HV condition detected
VMON_ISR_5			BCLR	ATDCMPHTL, #VMON_VBAT_CONVERSION   		;VBAT must be lower (or same) than threshold
