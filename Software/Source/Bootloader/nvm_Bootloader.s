...
;-------------- 
#ifndef NVM_BUF_SOZE
NVM_PBUF_SIZE		EQU	8*NVM_PBUF_ENTRY_SIZE	;default is 8 entries
#endif	
...
NVM_PBUF_ENTRY_SIZE	EQU	NVM_PHRASE_SIZE+2 	;buffer entry size [bytes]
	
;Firmware size
NVM_FIRMWARE_SIZE	EQU	NVM_FIRMWARE_END_LIN-NVM_FIRMWARE_START_LIN
			
;Offset of the sector status field (sector address corresponding to NVM_SECTOR_STAT
NVM_SECTOR_STAT_OFFS	EQU	NVM_FIRMWARE_START_LIN/(8*NVM_SECTOR_SIZE)
...
NVM_SECTOR_STAT		DS	NVM_FIRMWARE_SIZE/(8*NVM_SECTOR_SIZE)
NVM_SECTOR_STAT_END	EQU	*

NVM_PBUF		DS	NVM_PBUF_SIZE
NVM_PBUF_END		EQU	*
NVM_PBUF_IN		DS	1		;points to the next free space
NVM_PBUF_OUT		DS	1		;points to the oldest entry
...
#macro	NVM_INIT, 0
			...
			LDX	#NVM_SECTOR_STAT
			LDY	#(NVM_SECTOR_STAT_END-NVM_SECTOR_STAT)/4
			CLRA
			CLRB
LOOP			STD	2,X+
			STD	2,X+
...
NVM_PGM_PHRASE_NB	EQU	*
			...
			LDAA	NVM_PBUF_IN 				;IN -> A
			TAB						;IN -> B
			ADDA	#NVM_PBUF_ENTRY_SIZE 			;increment IN (A)
			CMPA	#NVM_PBUF_SIZE 				;check if wrapping is required
			BLO	NVM_PGM_PHRASE_NB_1 			;no wrapping required
			CLRA			    			;wrap IN (A)
NVM_PGM_PHRASE_NB_1	CMPA	NVM_PBUF_OUT 				;check if buffer is full
			BEQ	NVM_PGM_PHRASE_NB_2 			;buffer is full
			;Insert buffer entry (next IN in A, current IN in B)
			LDX	#NVM_PBUF 				;start of buffer -> X
...
NVM_ISR			EQU	*
			...
			LDD	NVM_PBUF_IN 			;IN-> A, OUT -> B
			CBA					;check if queue is empty
			BEQ	NVM_ISR_			;nothing to do
			;Check if sector has already been tpuched (OUT in B) 
			LDX	#(NVM_PBUF+NVM_PHRASE_SIZE) 	;first address field -> X 
...
NVM_SET_CHK_STAT	EQU	*
			...
			LDY	#NVM_SECTOR_STAT		;status field offset -> Y
